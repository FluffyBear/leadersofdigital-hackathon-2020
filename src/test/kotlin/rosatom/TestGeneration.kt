...
class TestGeneration {
    ...
    @Test
    fun generate() {
        File("src/main/resources/analyse-test/3.json").writeText(Json(JsonConfiguration.Stable)
    ...
    private fun generateJobs(): List<SimplifiedJobDto> {
        ...
        jobs.add(SimplifiedJobDto(991, "${991+0}", 1, 2))
        jobs.add(SimplifiedJobDto(992, "${991+1}", 2, 4))
        jobs.add(SimplifiedJobDto(993, "${991+2}", 2, 1))
        jobs.add(SimplifiedJobDto(994, "${991+3}", 4, 4))
        jobs.add(SimplifiedJobDto(995, "${991+4}", 3, 4))
        jobs.add(SimplifiedJobDto(996, "${991+5}", 7, 2))
        jobs.add(SimplifiedJobDto(997, "${991+6}", 9, 3))
        jobs.add(SimplifiedJobDto(998, "${991+7}", 10, 2))
        jobs.add(SimplifiedJobDto(999, "${991+8}", 5, 3))
        jobs.add(SimplifiedJobDto(1000, "${991+9}", 12, 1))
        for (i in 100 until 1001 step 200) {
            jobs.add(SimplifiedJobDto(1000 + i/100, "${1000 + i/100}", 15, 2))
        }
        for (i in 200 until 1001 step 200) {
            jobs.add(SimplifiedJobDto(1000 + i/100, "${1000 + i/100}", 15, 1))
        }
        jobs.add(SimplifiedJobDto(1011, "1011", 15, 2))
        return jobs
    }
